...
impl App {
    ...
    fn system_page(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some((settings, name)) = self.system.clone() {
            ...
            card(ui, |ui| {
                ...
                note(ui,lang.text("Disable acceleration + neutral speed. Saved presets include these settings; automatic switching restores them when focus leaves the game.","关闭加速 + 默认速度。保存预设时会包含这些设置；前台切出游戏后自动恢复进入前的设置。"));
                if ui
                    .add_enabled(
                        !self.busy,
                        egui::Button::new(
    ...
    fn desktop_page(&mut self, ui: &mut egui::Ui) {
        ...
        card(ui, |ui| {
            ...
            note(ui,lang.text("Opening MCHOSE again shows the existing window. Quitting stops automatic presets and restores the captured settings.","再次打开应用会唤回已有窗口。退出程序会停止自动切换，并恢复进入游戏前的设置。"));
            ui.add_space(12.0);
            if ui
    ...
    fn presets_page(&mut self, ui: &mut egui::Ui) {
        ...
        for (name, p) in self.presets.clone() {
            card(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if active {
                        ui.label(RichText::new(lang.text("Active", "当前匹配")).color(TEAL));
                    }
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui
        ...
        note(ui,lang.text("Built-in cs / desk presets preserve your grip angle. Save a custom preset to recall a particular angle. Built-ins cannot be deleted; deleting an override restores the built-in.","内置 cs / desk 预设保留你的握持角度。保存自定义预设即可同时恢复角度。内置预设不能删除；删除同名自定义配置会恢复内置值。"));
    }
}
impl App {
    ...
    fn auto_page(&mut self, ui: &mut egui::Ui) {
        ...
        let lang = self.language;
        card(ui, |ui| {
            title(ui,lang.text("Follow the foreground app","跟随前台应用"),lang.text("Match an app, apply its preset. Switch away to restore your previous settings.","前台应用命中规则时应用预设；切出后恢复此前的鼠标配置。"));
            ui.add_space(10.0);
            ui.horizontal(|ui| {
            ...
            note(ui,lang.text("Monitoring continues after this window closes. Turn it off here to stop and restore your settings.","关闭此窗口后仍会继续监听；在这里关闭自动切换，即可停止并恢复原配置。"));
        });
        ui.add_space(12.0);
        card(ui, |ui| {
            title(
            ...
            ui.columns(2, |cols| {
                ...
                if ui.add_enabled(self.app_selected.is_some() && valid && !self.demo,egui::Button::new(lang.text("Save binding", "保存绑定"))).clicked() {
                    let mut rules=self.auto_rules.clone();rules.push(auto::Rule{enabled:true,preset:self.rule_preset.clone(),filter:self.rule_filter.clone()});
                    match auto::save_rules(&rules){Ok(())=>{self.auto_rules=rules;self.auto_error=None;},Err(e)=>self.auto_error=Some(e.to_string())}
                }
                note(ui,lang.text("Switching away restores your previous mouse settings.", "切出匹配应用后，会恢复之前的鼠标设置。"));
                ui.add_space(14.0);
