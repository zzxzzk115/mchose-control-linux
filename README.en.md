...
Configure MCHOSE mice on Linux: DPI, grip-angle compensation, sensor settings, system pointer settings and foreground-application presets that restore your previous configuration when you switch away.
...
Captured from the running application on **KDE Plasma 6 / Wayland with a MCHOSE A7 Pro**, firmware **5.4.7.4**. These show actual read-back settings, not mockups or demo data. The displayed values are not recommendations.
...
- Named presets including optional rotation and system settings, with Unicode names.
- Installed application and Steam game picker; advanced identifier filters remain available. Uses the foreground application, restoring the captured baseline when focus leaves matching applications.
...
The GUI application picker avoids typing identifiers. Closing to the tray keeps monitoring active. **Quit and restore** stops automatic switching and restores the captured settings. Login startup is configured separately for the GUI and automatic preset service in Preferences.

The built-in `cs` preset uses 800 DPI, 1000 Hz, flat system acceleration and neutral system speed, preserving your current grip angle. Save your own preset to customize it. The −100…+100 speed scale maps to libinput −1…+1; it is neither a Windows speed level nor a percentage gain. Raw-input games may bypass desktop pointer settings; in-game sensitivity is separate.
...
LOD cannot be read from hardware and displays the last tool-written value. Sensor rotation is distinct from angle snapping. Normal automatic-controller shutdown restores the baseline; forced termination, power loss or device removal cannot guarantee restoration. No firmware writes are performed. Hardware and desktop tests do not verify the input pipeline inside CS itself.
