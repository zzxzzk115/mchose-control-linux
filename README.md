...
在 Linux 上调整迈从鼠标的 DPI、握持角度、传感器参数和系统鼠标设置，用前台应用绑定预设，切出游戏后应用 `desktop` 默认预设。
...
以下截图来自 **0.3.3 / KDE Plasma 6 / Wayland + MCHOSE A7 Pro** 的实际运行，固件 **5.4.7.4**，不是设计稿或演示数据。显示的参数是截图时的配置，不是推荐值。
...
- **预设**：独立编辑 `cs2`、`desktop` 及自定义预设，支持中文名称；保存前不写入鼠标。卡片展示应用图标、过滤条件和启用状态。
- **应用绑定**：选择已有应用或已安装的 Steam 游戏；高级用户可展开标识符过滤规则。以前台应用为准，切出后应用可编辑的 `desktop` 默认预设。
- **切换反馈**：成功读回后通知，失败原因和最近事件可查；参数一致与应用规则启用分别显示。
- **桌面集成**：原生托盘、关闭驻留、单实例唤回、登录自启。自启选项统一位于“桌面与驻留”。
- **GUI + CLI**：默认中文，可切换英文或跟随系统；JSON 输出和命令名保持稳定。

## 预设与自动切换

![真实预设、应用图标与匹配条件](docs/presets-zh.png)

- `cs2` 是游戏预设；`desktop` 是桌面默认预设。监听器在未匹配应用时启动，会立即应用 `desktop`，切出游戏或在游戏中停止监听也会切回它。
- 两者均可在“预设管理 → 编辑”修改 DPI、回报率、角度、传感器和系统鼠标参数。保存后覆盖内置值；删除自定义覆盖会恢复内置值。已有的同名自定义预设优先，旧 `cs` / `desk` 名称与绑定保持兼容。
- **参数相同**只表示当前鼠标参数与预设一致，**由应用规则启用**才表示前台规则正在使用该预设。点击“应用”手动设置鼠标，不代表游戏正在运行。
- 从已安装应用或 Steam 游戏中选择绑定；同一应用再次绑定会更新已有规则。第一条启用且命中的规则优先。应用图标取自本机，缺少可用图标时显示占位图标。
- 切换成功、切回桌面和失败都有通知及事件记录。KDE 勿扰或全屏策略可能隐藏弹窗，仍可在应用页面查看记录。
...
在 CachyOS 上，建议沿用系统自带的 `game-performance` 与 `ananicy-cpp`。Steam 的 CS2 启动参数：

```text
game-performance %command%
```

它随游戏进程启用性能电源配置、退出后恢复；切出游戏不会关闭性能模式。其他发行版可以考虑 [Feral GameMode](https://github.com/FeralInteractive/gamemode)。不要直接叠加两套调度配置，见 [CachyOS 游戏指南](https://wiki.cachyos.org/configuration/gaming/)。仅用 Steam 玩 CS2 不需要额外安装 Lutris。

附带独立、可选的 **CS2 Meta 保护工具**：CS2 在前台时暂停含 Meta 的 KDE 桌面键盘快捷键，保留 Alt+Tab 和非 Meta 绑定，切出后恢复。它不需要鼠标监听器运行，也不是屏蔽所有快捷键的窗口规则。需要 KDE Plasma 6、Python 3 / Gio 和用户 systemd；不属于默认安装内容。[安装、停用与恢复说明](docs/gaming.md)。
...
普通用户可在“应用自动切换”选择应用和预设，无需手写标识符。`auto stop` 会停止监听并应用 `desktop` 默认预设。**关闭到托盘**继续运行；**退出并恢复**会停止自动切换并应用 `desktop`。

内置 `cs2` 预设含 800 DPI、1000 Hz、关闭系统加速和系统速度 0，保留当前握持角度；可直接编辑并保存，无需先写入鼠标。系统速度刻度 −100～100 对应 libinput −1～1，不是 Windows 档位，也不是百分比增益。使用原始输入的游戏可能绕过系统指针设置，游戏内灵敏度单独调整。
...
LOD 不能从设备读回，显示的是工具上次写入值。握持角度旋转与直线修正是两种独立功能。自动切换正常退出时应用 `desktop`；强制杀进程、断电或设备断开无法保证恢复。程序不写固件。请勿将本机测试视为所有型号或 CS 游戏内输入链路的验证。

详细说明：[中文使用指南](README.zh-CN.md) · [协议文档](PROTOCOL.md) · [协议分析工具](protocol/README.md)

## 开发

0.3.3 修复监听启动时遗漏桌面默认预设；0.3.2 修复 A7 Pro 固件自管状态位引起的误报失败，以及 GUI 忙碌时遗漏自动刷新。已有 23 项 Rust 测试，并在 A7 Pro / KDE Wayland 上验证预设写入、启动默认、切出恢复、通知与 Meta 保护。窗口联动测试使用临时测试窗口，不等同于 CS2 对局性能测试。
