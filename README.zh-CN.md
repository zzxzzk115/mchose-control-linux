...
- 内置 `cs2` 和 `desktop` 预设**保留当前角度**。自定义预设保存并恢复角度。
...
- **预设管理**：独立编辑预设、保存当前设置、应用和删除自定义覆盖，支持中文名称。
...
CS 在前台且命中规则时应用游戏预设；切到未匹配应用后应用 `desktop` 默认预设；监听在桌面启动时也会立即应用它。
多个匹配预设连续切换时，未指定的角度和系统参数共用首次进入前的基线；切出后的目标始终是 `desktop`。

```sh
mchose auto bind cs2 'app_id=steam_app_730 || class=steam_app_730 || exe=cs2'
mchose auto start
mchose auto status
mchose auto stop
```
...
mchose auto bind-app org.kde.konsole desktop
```
...
- 后台监听独立于 GUI，关闭 GUI 不会停止；关闭“启用自动切换”或运行 `auto stop` 才停止，并应用 `desktop` 默认预设。
...
内置 `cs2` 预设现在包含 **关闭系统加速 + 速度 0**，因此应用它也要求系统接口可用。旧预设不含系统字段时仍保留系统设置。保存新预设会包含可读取的系统速度与加速度。自动切换在进入匹配应用前记录系统和硬件设置，切出、暂停或正常退出时应用 `desktop`；切换到未指定系统参数的另一条规则时沿用进入前的基线。原始输入游戏可能绕过系统指针设置，游戏内灵敏度单独调整。
...
当前版本的状态标签、通知和可选游戏集成见 [主 README](README.md) 与 [游戏配置指南](docs/gaming.md)。
